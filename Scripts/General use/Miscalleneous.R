...
# Produce a sample made up of specified elements, that has a specified length
sample(c("Bla","Bli","Blu"),10,replace = TRUE)

# Remove all items from the environment matching the specificed pattern
rm(list=ls(pattern="rain")) # Remove all items whose name include "rain"
...
# Create a new variable based on the values of other variables with mutate and case_when
data = data %>% 

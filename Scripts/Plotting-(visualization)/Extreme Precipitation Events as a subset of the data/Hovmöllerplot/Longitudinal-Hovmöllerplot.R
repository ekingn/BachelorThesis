# The purpose of this script is to create a longitudinal Hovmöllerplot, which is a special type of heatmap.
# A longitudinal Hovmöllerplot is a grid consisting of fields, where each field corresponds to a combination 
# of longitudinal degree and a variable of time, in this case year. Since the initial weather data is not grid, but point data, it had to be 
# translated to grid data first. To that end, first of all, the number of longitudinal grid points was set 
# to 125, that range between the minimum of longitudinal degree and the maximum of longitudinal degree in the data (in the case of longitude)
...
# point datum is allocated to the grid point, to whom the longitudinal degree of the point data has the closest distance. 
# The number of temporal (year) grid points was naturally set to 21, the number of years observed in the data. 
# From 125 longitudinal grid-points and 21 temporal grid-point follow 2625 groups of data or 2625 longitude-year grid-fields. 
# After having the weather data seperated into 2625 groups, the average share of EPE within this group is computed, #
# which is an average across all weather stations within that longitudinal group for a particular year. 
...
Range_Longitude_Degrees = range(Weather_Data_Daily_Resolution_Without_NA_Precipitation$lon)
...
Space_Time_Grid = expand.grid(longitude = Spatial_Axis,
                              time = Temporal_Axis)
...
distances = abs(outer(Weather_Data_Daily_Resolution_Without_NA_Precipitation_Grid$lon, Spatial_Axis, "-"))

### Create a gridded version of the spatio-temporal data
Weather_Data_Daily_Resolution_Without_NA_Precipitation_Grid$lon = Spatial_Axis[apply(distances, 1, which.min)]
...
longitudinal_Hovmoellerplot = ggplot(Dataframe_Hovmoellerplot) +
...
save_plot(plot =  longitudinal_Hovmoellerplot,
          filename = "Longitudinal-Hovmoellerplot-share_of_EPE.JPG",

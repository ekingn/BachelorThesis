...
\center{\fbox{\textbf{Logistic Regression Model}}}
...
\begin{itemize}
 \item One spatial and one temporal Semivariogram
 \item One spatial and one temporal Covariancematrix
 \item Spatial autocorrelation measured as spatial covariance as a function of latitudinal lag d
 \item Temporal autocorrelation measured as temporal covariance as a function of temporal lag p
\end{itemize}

## Spatial Semivariance Equation (for Semivariogram)

$$\gamma_{\mathrm{lon}}(d) = \frac{1}{2N(d)(T-p)}{}\sum_{i=1}^{N(d)}\sum_{t=p+1}^{T}(rain_{\mathrm{\small{lon}},\small{lat_i+d,t}} - rain_{\mathrm{\small{lon}},\small{lat_i,t-p}})^2$$

## Spatial Covariance Equation

$$ \hat{C}_{\mathrm{lon}}(lat_i,lat_k) = \frac{1}{T-p}\sum_{t=p+1}^{T} \left(rain_{\mathrm{lon},lat_i,t} - \frac{1}{T_{\mathrm{lat_i}}}\sum_{t=1}^{T_{\mathrm{lat_i}}} rain_{\mathrm{lon},lat_i,t}\right)\left(rain_{\mathrm{lon},lat_k,t-p} - \frac{1}{T_{\mathrm{lat_k}}}\sum_{t=1}^{T_{\mathrm{lat_k}}} rain_{\mathrm{lon},lat_k,t-p}\right) $$
\center{\textbf{where $lat_i$ and $lat_k$ are latitudinal degrees, such that ($\mathrm{lat_i}$,$\mathrm{lat_k}$) $\epsilon$ [47.4,54.8]}}

\center{\textbf{and where $\mathrm{lon}$ denotes one of five latitudinal intervals of equal length, as follows:}}

\begin{itemize}
\item Interval 1: 6.02 to 7.49 degrees longitude\footnote{If the observation belongs to interval 1, then $\mathrm{lon}=1$.}
\item Interval 2: 7.49 to 8.96 degrees longitude\footnote{If the observation belongs to interval 2, then $\mathrm{lon}=2$.}
\item Interval 3: 8.96 to 10.43 degrees longitude\footnote{If the observation belongs to interval 3, then $\mathrm{lon}=3$.}
\item Interval 4: 10.43 to 11.9 degrees longitude\footnote{If the observation belongs to interval 4, then $\mathrm{lon}=4$.}
\item Interval 5: 11.9 to 13.37 degrees longitude\footnote{If the observation belongs to interval 5, then $\mathrm{lon}=5$.}
\item Interval 6: 13.37 to 14.95 degrees longitude\footnote{If the observation belongs to interval 6, then $\mathrm{lon}=6$.}
\end{itemize}

## Spatial Latitudinal Autocorrelation Equation 

\begin{align*}
\hat{C}_{\mathrm{lon}}(d,p) &= \frac{1}{T-p}\sum_{t=p+1}^{T} \sum_{\mathrm{\scriptsize{lat_i}}} \sum_{\mathrm{\scriptsize{lat_k}}} \left(rain_{\mathrm{\scriptsize{lon}},\mathrm{\scriptsize{lat_i}},t} - \frac{1}{T_{\mathrm{\scriptsize{lat_i}}}}\sum_{t=1}^{T_{\mathrm{\scriptsize{lat_i}}}} rain_{\mathrm{\scriptsize{lon}},\mathrm{\scriptsize{lat_i}},t}\right) \\
&\qquad \times \left(rain_{\mathrm{\scriptsize{lon}},\mathrm{\scriptsize{lat_k}},t-p} - \frac{1}{T_{\mathrm{\scriptsize{lat_k}}}}\sum_{t=1}^{T_{\mathrm{\scriptsize{lat_k}}}} rain_{\mathrm{\scriptsize{lon}},\mathrm{\scriptsize{lat_k}},t-p}\right) \delta_{\mathrm{\scriptsize{lat_i-d}},\mathrm{\scriptsize{lat_k}}}
\end{align*}

...
&\qquad + \beta_3 \textbf{Longitude} + \beta_4 \textbf{Latitude} + \beta_5 \textbf{Julian Date} + \mathbf{\epsilon}
\end{align} 
...
\vspace{-35pt}
\begin{equation}
\textbf{p(PE in class EPE)} = 
\scalebox{2}{s}\left(log\left(\frac{p}{1-p}\right)\right) = 
\scalebox{2}{s} \left(
\begin{aligned}
& \mathbf{\beta_0} + \vphantom{\sum} \beta_1 \textbf{Mean Temperature} + \beta_2 \textbf{Altitude} \\
& \qquad + \beta_3 \textbf{Longitude} + \beta_4 \textbf{Latitude} \\
& \qquad\qquad + \beta_5 \textbf{Julian Date}
\end{aligned}
\right)
\end{equation}
\vspace{-35pt}
...
\scalebox{1.25}{$\beta_0,\beta_1,\beta_2,\beta_3,\beta_4$ are \textit{estimated} per \textit{Maximum Likelihood Estimation}},

$P(EPE_i | \textbf{x}_i, \beta) = p_i^{EPE}(1-p_i)^{1-EPE}$ is the likelihood of a single classification decision 

$p_i = \frac{1}{1 + e^{-(\beta_0 + \beta_1x_{i1} + \beta_2x_{i2} + \beta_3x_{i3} + \beta_4x_{i4} + \beta_5x_{i5})}}$ is the predicted probability, that an EPE occurs at spatio-temporal location i 

$L(\beta_0, \beta_1, \beta_2, \beta_3, \beta_4, \beta_5) = \prod_{i=1}^{n} P(EPE_i | \textbf{x}_i, \beta) = \prod_{i=1}^{n} p_i^{EPE_i}(1-p_i)^{1-EPE_i}$
...
\justify{As a part of the general argument of this thesis, I had asserted, that there are spatial, temporal and spatio-temporal dependencies within the precipitation height data, which should be accounted for conceptually, when modelling the classification procedure. In this section, you can find equations, on which the calculation of the variation of covariability is based, which is then visualized and analyzed for the reason of demonstration of the earlier mentioned dependencies. Reflecting the different types of spatial, temporal and spatio-temporal covariability, the equations for the $\mathbf{empirical}$ $\mathbf{spatial}$ and $\mathbf{temporal}$ $\mathbf{Semivariogram}$, the $\mathbf{spatial}$ and $\mathbf{temporal}$ $\mathbf{covariancefunction}$ and the $\mathbf{spatio-temporal}$ $\mathbf{covariancefunction}$ are depicted and described in detail.} In this context, spatial, temporal and spatio-temporal dependency are understood to be patterns, where the similarity between observations decrease, as the distance increases between the obersvations's locations increase. These measures then can be calculate the variation in the covariability of the precipitation height data between a pair of spatial, temporal or spatio-temporal locations, as the distance between the paired locations change. As covariability is commonly used to indicate similarity in data, the variation of covariability as a function of spatial, temporal or spatio-temporal lags can hence be utilized for the assessment of the prevelance of the various types of dependencies.          

### Haversine Formula: Calculating the distance between two pairs of angular coordinates

A necessary requisite for the calculation of the variation of covariability however is to determine a concept for the various lags first, or in other words to decide upon a way to measure distance. For the spatial distance between two weather stations, the Haversine Formula is used to measure the distance between two points on the Earth's surface, based on angular coordinates. The Haversine Formula takes the curvature of the Earth's Surface into account and provides an estimate of the socalled great-circle distance between two points. With reference to quality of measurement concept, it exceeds for instance the euclidean distance, which assumes a flat sphere.
...
\begin{align}
\gamma(d_{spatial},jul_a) &= \frac{1}{2|N(d_{spatial})|}\times\left(\sum_{(lon_r,lat_i),(lon_s,lat_k)\in\:N(d_{spatial})}\left(rain_{lon_r,lat_i,jul_a} - rain_{lon_s,lat_k,jul_a}\right)^2\right)
\end{align}
...
\begin{adjustwidth}{1cm}{0cm} An interval of spatial distance between weather stations measured in kilometre within which the rainfall data are considered to be correlated. \end{adjustwidth}
...
\center{\rule{1\linewidth}{0.5pt}}
\begin{align}
\gamma(d_{julian},(lon_r,lat_i)) = \frac{1}{2|N(d_{julian})|}\times\left(\sum_{jul_a,jul_b\:\in\:N(d_{julian})}\left(rain_{lon_r,lat_i,jul_a} - rain_{lon_r,lat_i,jul_b}\right)^2\right)
\end{align}
...
\center{\rule{1\linewidth}{0.5pt}}
\begin{align}
\hat{C}_{rain}^{\left(d_{julian}\right)}\left(\left(lon_r,lat_i\right),\left(lon_s,lat_k\right)\right)&=\frac{1}{T-d_{julian}}\times\Bigg(\sum_{jul_a=1+d_{julian}}^{T}\left(rain_{lon_r,lat_i,jul_a}-\hat{\mu}_{rain,temporal}(lon_r,lat_i)\right) \\ 
&\qquad\left(rain_{lon_s,lat_k,jul_a-d_{julian}}-\hat{\mu}_{rain,temporal}(lon_s,lat_k)\right)\Bigg)
\end{align}
\center{\rule{1\linewidth}{0.5pt}}
...
\begin{itemize}[label={}]
            \item$\mathbf{\hat{C}_{rain}^{(d_{julian})}}$:
            ...
            \item $\mathbf{\hat{\mu}_{rain,spatial}(lon_r,lat_i)}$ and $\mathbf{\hat{\mu}_{{rain,spatial}}(lon_s,lat_k)}$:\\ 
...
\center{\rule{1\linewidth}{0.5pt}}
\begin{align}
\hat{C}_{rain}^{\left(d_{spatial}\right)}\left(jul_a,jul_b\right)&=\frac{1}{|N(d_{spatial})|}\times\Bigg(\sum_{(lon_r,lat_i),(lon_s,lat_k)\:\in\:N(d_{spatial})}\left(rain_{lon_r,lat_i,jul_a}-\hat{\mu}_{rain,spatial}(jul_a)\right) \\ 
&\qquad\left(rain_{lon_s,lat_k,jul_b}-\hat{\mu}_{rain,spatial}(jul_b)\right)\Bigg)
\end{align}
\center{\rule{1\linewidth}{0.5pt}}
...
\begin{itemize}[label={}]
\item $\mathbf{\hat{C}{rain}^{\left(d{spatial}\right)}}$:
\begin{adjustwidth}{1cm}{0cm}
...
\end{adjustwidth}
\item $\mathbf{\hat{\mu}_{rain,temporal}(jul_a) \text{ and } \hat{\mu}_{rain,temporal}(jul_b)}$:
\begin{adjustwidth}{1cm}{0cm}
...
\center{\rule{1\linewidth}{0.5pt}}
\begin{align}
\hat{C}_{rain}(d_{spatial},d_{julian}) &= \frac{1}{2|N(d_{spatial})|}\frac{1}{2|N(d_{julian})|}\times \Bigg(\sum_{(lon_r,lat_i),(lon_s,lat_k)\:\in\:N(d_{spatial})}\:\sum_{jul_a,jul_b\:\in\:N(d_{julian})} \\ &\qquad\left(rain_{lon_r,lat_i,jul_a} - \hat{\mu}_{jul_a}\right)\left(rain_{lon_s,lat_k,jul_b} - \hat{\mu}_{jul_b}\right)\Bigg)
\end{align}
\center{\rule{1\linewidth}{0.5pt}
...
\vspace{-0.5cm}
  \begin{itemize}[label={}]
              \item $\mathbf{\hat{C}_{rain}(d_{spatial},d_{julian})}$: \\ 
              ...
              \item $\mathbf{rain_{lon_r,lat_i,jul_a}}$:\\ 
                ...
                \end{adjustwidth}
              \item $\mathbf{\hat{\mu}_{rain,temporal}(jul_a)}$ and $\mathbf{\hat{\mu}_{rain,temporal}(jul_b)}$:\\
                \begin{adjustwidth}{1cm}{0cm} The spatial mean of the rainfall data at time $jul_a$ and $jul_b$.
                \end{adjustwidth}
              \item $\mathbf{(rain_{lon_r,lat_i,jul_a} - \hat{\mu}_{rain,temporal}(jul_a))}$:\\ 
                \begin{adjustwidth}{1cm}{0cm} The deviation of the rainfall data from their mean at location $(lon_r,lat_i)$ and time $jul_a$.
                \end{adjustwidth}
              \item $\mathbf{(rain_{lon_s,lat_k,jul_b} - \hat{\mu}_{rain,temporal}(jul_b))}$:\\ 
                \begin{adjustwidth}{1cm}{0cm} The deviation of the rainfall data from their mean at location $(lon_s,lat_k)$ and time $jul_b$.
                \end{adjustwidth}
              \item The product of the two deviations:\\
                \begin{adjustwidth}{1cm}{0cm} $(rain_{lon_r,lat_i,jul_a} - \hat{\mu}_{rain,temporal}(jul_a))(rain_{lon_s,lat_k,jul_b} - \hat{\mu}_{rain,temporal}(jul_b))$.
                \end{adjustwidth}
